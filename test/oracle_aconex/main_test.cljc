(ns oracle-aconex.main-test
  "Contract + behavioral test for the oracle-aconex-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "oracle_aconex-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'oracle-aconex.main-test)]
