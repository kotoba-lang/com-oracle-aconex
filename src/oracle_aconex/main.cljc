(ns oracle-aconex.main
  "Kotodama WASM entrypoint for the Oracle_aconex clean-room actor (L4) — Clojure port.
  ...
  facts (`oracle_aconex.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "oracle_aconex")
(def tier "L4")
...
(def entity-specs
  [{:entity "Project"      :plural "projects"      :id-prefix "oracleac_pro"
    :fields [:name :jurisdiction :status]  :required [:name :jurisdiction]
    :coerce {}                              :refs {}}
   {:entity "Permit"       :plural "permits"       :id-prefix "oracleac_per"
    :fields [:projectId :type :number :status]     :required [:type :number]
    :coerce {}                              :refs {:projectId "Project"}}
   {:entity "Drawing"      :plural "drawings"      :id-prefix "oracleac_dra"
    :fields [:projectId :name :revision :contentRef] :required [:name :revision]
    :coerce {}                              :refs {:projectId "Project"}}
   {:entity "Inspection"   :plural "inspections"   :id-prefix "oracleac_ins"
    :fields [:permitId :type :result :inspectedAt] :required [:type :result]
    :coerce {}                              :refs {:permitId "Permit"}}
   {:entity "Asset"        :plural "assets"        :id-prefix "oracleac_ass"
    :fields [:name :type :location :condition]     :required [:name :type]
    :coerce {}                              :refs {}}
   {:entity "ServiceRequest" :plural "servicerequests" :id-prefix "oracleac_ser"
    :fields [:citizenId :category :status]         :required [:category :status]
    :coerce {}                              :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"oracle_aconex.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "oracle_aconex-compat" :tier tier :entities entities} 200])
...
(defn start! [] :oracle_aconex-compat/ready)
